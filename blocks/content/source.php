...
<p>
  With the JavaScript system described, the CSS portion is straightforward. The same tool mentioned above
  minifies the CSS. If <?=code('DEBUG')?> is nonzero, the non-minified development CSS files are used. If
  <?=code('DEBUG')?> is <?=code('0')?>, the minified CSS files generated by the admin tool are used.
</p>
<p>
...
<p>
  ...
  It's worth noting I also set my webhost to redirect <?=code('afeique.com')?> to <?=code('www.afeique.com')?> in order 
  that <?=code('static.afeique.com')?> could be a cookie-less domain. Otherwise, cookies set from people accessing the 
